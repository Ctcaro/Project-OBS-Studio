...
static bool init_update(struct update_info *info)
{
	...
	info->local_package = get_package(info->local, "services.json");
	info->cache_package = get_package(info->cache, "services.json");
...
static int update_local_version(struct update_info *info)
{
	...
	if (cache_version < local_version) {
		...
		copy_local_to_cache(info, "services.json");
...
////////////////////
static bool do_http_request_new(struct update_info *info, const char *url,
	long *response_code)
{
	CURLcode code;
	uint8_t null_terminator = 0;
...
static void updateremote(struct update_info *info, int cur_version)
{
	int remote_version;
	long response_code;

	if (!do_http_request_new(info, "http://localhost/services.php", &response_code))
		return;
	...
	write_file_data(info, info->temp, "services.json");
	...
	replace_file(info->temp, info->cache, "services.json");
	info("Successfully updated package (version)");
	return;
}
