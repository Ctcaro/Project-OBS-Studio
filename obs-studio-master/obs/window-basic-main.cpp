...
//	connect(ui->VetvPreview, SIGNAL(clicked()), this, SLOT(on_VetvPreview_clicked()));
...
void OBSBasic::on_VetvPreview_clicked()
{
	QSettings settingdefault("HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\Shell\\Associations\\UrlAssociations\\http\\UserChoice", QSettings::NativeFormat);
	QSettings settings1("HKEY_LOCAL_MACHINE\\SOFTWARE\\Clients\\StartMenuInternet", QSettings::NativeFormat);
	QSettings cocCoc("HKEY_CURRENT_USER\\Software");
	QMap<QString, QString> map;
	map.insert("ChromeHTML", "Google Chrome");
	map.insert("FirefoxURL", "FIREFOX.EXE");
	//map.insert("ChromeHTML", "IE.HTTP");
	
	QString str1 = "Progid";
	//QString str2 = ""
	//settings.finc
	QString defaultbrowser = settingdefault.value("Progid").toString();
	
	
	if (defaultbrowser == "IE.HTTP") {
		QMessageBox::information(this, "Error", "Our Player doesn't support Internet Explorer.Please install another browser");
	}
	else if (defaultbrowser == "ChromiumHTM.WGXGAFDX3I23G35FHZFO53H4R4"){
		if (cocCoc.childGroups().contains("CocCoc", Qt::CaseInsensitive)){
			QMessageBox::information(this, "Error", "Cannot find default browser. Please set your default browser");
			return;
		}
			
	}else if (defaultbrowser=="")
	{
		QMessageBox::information(this, "Error", "Please set your default browser");
		return;
	}
	
		
	
	bool bstr = settings1.childGroups().contains(map.value(defaultbrowser), Qt::CaseInsensitive);
	//QString str = settings1.value("Default").toString();
	//bool bstr = settings1.childGroups().contains("Google Chrome", Qt::CaseInsensitive);
	if (bstr)
	QMessageBox::information(this, "Error", "Cannot find default browser. Please set your default browser!.");
	if(QDesktopServices::openUrl(QUrl("http://beta.vetv.vn"))) return;
	QMessageBox::information(this, "Error", "Cannot find default browser. Please set your default browser!.");
	/*QProcess *process = new QProcess(this);
	QString wordPath = "C:\\Program Files (x86)\\Internet Explorer\\iexplore.exe";
	process->start(wordPath, QStringList() << "beta.vetv.vn");*/
}
